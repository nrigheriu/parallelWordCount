...
public class WoCoClient {
	...
	public float calculateAverageResponseTime(ArrayList<Float> respTime) {
        float arraySum = 0;
        for (int i = 0; i < respTime.size(); i++) {
			arraySum += respTime.get(i);
			System.out.println(arraySum);
		}
        return arraySum / (float) respTime.size();
    }
	/**
	...
	public void printStats(boolean withPercentiles) {
		...
		cntSincePrint = 0;
		System.out.println("Average response time:" + calculateAverageResponseTime(respTime));
		if (withPercentiles) {					//sorting for pctiles
	...
	public static void main(String[] args) throws UnknownHostException, IOException, InterruptedException {
		...
		for (int rep=0; rep<ops; rep++) {
			...
			if (rep%25 == 0) 				//reduce the overhead of printing statistics by calling this less often
